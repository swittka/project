...
instead of an 32 bit one use \lstinline$--64bit$. If only the adapters for UVM-SV, UVM-SystemC and
...
unit needs to include an proxy unit for the foreign component (see example~\ref{lst:e_unified_top}).
...
declaring an component of type \lstinline$uvm_component$ inside of the parent
...
the value from its database. When it is an primitive data type like an integer as in example~\ref{lst:SV_get_int},
...
module tb;
  ...
  bit res = set_vif(if);|\label{line_sv_call_function_vif}|
endmodule : tb
\end{lstlisting}
...
class tb extends uvm_env;
  uvm_component e_uvc;
  //use uvm_component_uitls macro here and implement the constructor
...
In example~\ref{lst:SV_TLM_consumer} is the implementation of an TLM implementation port in UVM-SV
...
UVM-SV or UVM-SystemC test component or the full path of an file containing an UVM-\textit{e} test unit. Both arguments
