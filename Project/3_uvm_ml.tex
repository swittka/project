...
This paper focuses on the two verification languages SystemVerilog and \textit{e} both using the \emph{Universal
...
\begin{itemize}
  ...
  examples are UVM-SystemVerilog, UVM-SystemC and UVM-\textit{e}.
...
of an interface UVC implemented in UVM-SystemVerilog and a module UVC in
UVM-\textit{e}. \\
...
instantiates an UVM-SystemVerilog component within an UVM-\textit{e} unit and vice versa.
...
\paragraph{Instantiating an UVM-SystemVerilog Component within an UVM-\textit{e} Unit}
\label{sv_inside_e}
When instantiating an UVM-SystemVerilog component within an UVM-\textit{e} unit,
it is necessary to instantiate a proxy unit inside the UVM-\textit{e} unit. After that the
proxy unit and the UVM-SystemVerilog component need to be connected. \\
An example for the code of the extended UVM-SystemVerilog component can be seen in
example~\ref{lst:SV_unified_sub}.
It is recommended to extend the UVM-SystemVerilog component
...
package for UVM-SystemVerilog (line~\ref{line_uvm_ml_package}).
...
\end{lstlisting}
\medskip
Corresponding to the extensions in the UVM-SystemVerilog component the UVM-\textit{e}
unit needs to include an proxy unit for it (see example~\ref{lst:e_unified_top}).
...
\begin{lstlisting}[frame=htrbl, caption={\textit{e}: instantiating the UVM-SystemVerilog component in the UVM-\textit{e} unit},
...
\paragraph{Instantiating an UVM-\textit{e} Unit Within an UVM-SystemVerilog Component}
When reusing an UVM-\textit{e} unit inside of an UVM-SystemVerilog component it is
necessary to integrate an proxy for the e unit within the UVM-SystemVerilog
...
database (see section~\ref{ml_config}), which gives the ability to customize the structure of the multi-language
...
For this purpose both, the UVM-SystemVerilog multi-language adapter as well as UVM-\textit{e} provide a function respectively
...
the UVM-SystemVerilog component (example~\ref{lst:SV_type_mapping}, line~\ref{line_sv_mapping}) or the
...
\subsubsection{\textit{e} Units Configuring UVM-SystemVerilog Components}\label{e_config_sv}

When configuring UVM-SystemVerilog components via UVM-\textit{e} units it is necessary to distinguish two cases. Firstly
...
\paragraph{Configuring UVM-SystemVerilog Components using Primitive Data Types}\label{config_sv_primitive}

When data is passed from an UVM-\textit{e} unit to an UVM-SystemVerilog component,
...
After the configuration is set in the database by the UVM-\textit{e} unit, the UVM-SystemVerilog component needs to get
the value from its database. When it has an primitive data type like an integer as in example~\ref{lst:SV_get_int},
...
\paragraph{Configuring UVM-SystemVerilog Components using Objects}\label{e_config_sv_object}

When using configuration objects instead of primitive data types, the UVM-\textit{e} unit can access its configuration
database the same way as with an primitive data type as shown in example~\ref{lst:e_set_object}. In this example
...
\end{lstlisting}
The difference between configuring an UVM-SystemVerilog component using a primitive data type compared to an object
...
Similar to section~\ref{e_config_sv} UVM-SystemVerilog components can configure UVM-\textit{e} units via the
configuration database. The syntax remains the same, just the part of getting respectively setting is switched for both
frameworks. The field that should be configured has to be registered in the database from the UVM-SystemVerilog
...
When configuring an UVM-\textit{e} unit via an UVM-SystemVerilog component, the component needs to register the value in
...
that shows the UVM-SystemVerilog component, which configurates a field of type integer within an UVM-\textit{e} unit. Firstly
...
Like UVM-SystemVerilog components, UVM-\textit{e} units can be configured via objects. An example for registering an
object in the configuration database via an UVM-SystemVerilog component is described in
...
Similar to the UVM-SystemVerilog component, the type of the configuration object needs to be declared in the
...
are registered at their framework adapter. For UVM-SystemVerilog this is done by calling
\lstinline$uvm_ml::tlm1::register()$ and for UVM-\textit{e} the port is bind to external by constraining \lstinline$bind()$.
Finally the ports are connected via \lstinline$uvm_ml::connect()$ for UVM-SystemVerilog or
...
Following examples for sending transactions from UVM-\textit{e} units to UVM-SystemVerilog components and
vice versa are provided to show, how to utilize the above introduced TLM functionalities for multi-language
environments.
\subsubsection{\textit{e} Units Sending Transactions to UVM-SystemVerilog Components}
In example~\ref{lst:SV_TLM_consumer} is the implementation of an TLM implementation port in UVM-SystemVerilog
displayed. After declaring (line~\ref{line_sv_imp_declare}) and instantiating (line~\ref{line_sv_imp_inst}) the port, it
is registered at the UVM-SystemVerilog multi-language adapter (line~\ref{line_sv_imp_reg}).\\
...
\end{lstlisting}
Finally the name of the sequence, which should be started, has to be registered in the configuration database by the top level UVM-SystemVerilog component (example~\ref{lst:SV_set_main_sequence}). This is done in line~\ref{line_sv_set_main_seq}, where the name of the sequence is registered for all sub-components, which end with the string \emph{'driver'}.
\lstset{language=SystemVerilog, numbers = left, escapechar=|, breaklines=true}
...
The above mentioned steps for executing multiple foreign sequences in a multi-language environment are now discussed in particular for UVM-\textit{e} sequences executing foreign UVM-SystemVerilog sub-sequences and sequence items.
\subparagraph{Extending the UVM-SystemVerilog Sequencer by a TLM Interface}
When extending the UVM-SystemVerilog Sequencer by a TLM interface, the UVM-ML package provides a template class for this sequencer TLM interface, located at:
\smallskip
\lstset{language={}, numbers=none, escapechar=|}
\begin{lstlisting}
...
\end{lstlisting}
After the TLM interface is implemented, it has to be integrated into the sequencer (example~\ref{lst:SV_sequencer_tlm}). To achieve this the sequencer is extended by a handle for the TLM interface (line~\ref{line_sv_declare_sequencer_tlm}). Followed by creating the interface via the UVM-SystemVerilog factory in line~\ref{line_sv_create_sequencer_tlm}. At last a reference of the sequencer is added to the TLM interface, so the received sequences and sequence items can be forwarded to the sequencer. 
\lstset{language=SystemVerilog, numbers = left, escapechar=|, breaklines=true}
...
After extending the foreign sequencer by a TLM interface, creating a proxy sequencer in UVM-\textit{e} and connecting both, the environment is now ready to execute UVM-SystemVerilog sequence items via an UVM-\textit{e} sequence. To be more specific, the sequence creates a proxy sequence item and passes it to the sequencer proxy. After that the item is propagated to the remote sequencer via the TLM interface and is then used as an native sequence item inside of the UVM-SystemVerilog component.\\
...
\lstset{language=e, numbers = left, escapechar=|, breaklines=true}
\begin{lstlisting}[frame=htrbl, caption={\textit{e}: sending an UVM-SystemVerilog sequence item  from  an UVM-\textit{e} sequence},
...
\end{lstlisting}
Now the foreign UVM-SystemVerilog sequences can be used like they were native UVM-\textit{e} sequences by executing the sequences, which extend the base sequence of the proxy sequencer and thereby act as proxy sequences.
\paragraph{UVM-SystemVerilog Sequences executing UVM-\textit{e} Sequences and Items}
Like all the other multi-language functionalities, sequence layering is not limited to UVM-\textit{e} sequences executing foreign UVM-SystemVerilog sub-sequences, but can also be used with switched roles. This means, that UVM-\textit{e} sub-sequences are utilized within UVM-SystemVerilog sequences. The same steps as mentioned previously are required to use multi-language sequence layering in this reversed configuration. 
\subparagraph{Extending the UVM-\textit{e} Sequencer by a TLM Interface}\label{e_seqr_tlm}
Firstly the template for TLM interfaces of UVM-\textit{e} sequencers has to be imported, which is located at:
\smallskip
\lstset{language={}, numbers=none, escapechar=|}
\begin{lstlisting}
...
UVM-SystemVerilog or UVM-SystemC test component or the full path of an file containing an UVM-\textit{e} test unit. Both arguments
