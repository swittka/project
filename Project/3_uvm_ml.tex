...
example, when verification projects are normaly implemented in \textit{e}, but some components are intellectual property
and provide UVCs implemented in \emph{SystemVerilog}. This leads to the demond of re-implementing fully operative code
just for switching the verification language.\\
This time consuming approach to this problem can be avoided by using the \emph{Universal Verification Methodology
...
integrade multi-language functionalities into \emph{Incisive Enterprise Simulator}. Followed by the illustration on how
...
\begin{itemize}
  ...
  The backplane is the central component of the architecture and connects the frameworks in a star topology. It is only
  internal to UVM-ML and cannot be accessed directly by the user code.
  \item\textbf{Adapter}\\
  Adapters build the bridge between frameworks and the backplane. It provides all multi-language functionalities to the
  framework so it is unaware of the backplane.
\end{itemize}
